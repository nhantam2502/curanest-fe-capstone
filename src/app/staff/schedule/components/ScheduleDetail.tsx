...
const EventDetailsDialog: React.FC<EventDetailsDialogProps> = ({
  ...
  const handleConfirmArrival = useCallback(async () => {
    ...
    setShowConfirmArrivalDialog(false); // Close confirmation dialog first
    try {
      ...
      if (response.status === 200 || response.status === 204) {
        toast({
          ...
          className: "bg-green-100 text-green-800", // Consider a success variant if available
        });
        onOpenChange(false); // Close the main details dialog
        if (onAppointmentUpdated) {
          onAppointmentUpdated(); // Trigger refetch in parent component
        }
      } else {
        throw new Error(
          response.payload?.message ||
      ...
      <AlertDialog open={isOpen} onOpenChange={onOpenChange}>
        <AlertDialogContent className="max-w-lg">
          ...
          <AlertDialogFooter className="mt-4">
            ...
            {event.status === "confirmed" && ( // Conditional rendering for the "Xác nhận đã đến" button
              <Button
